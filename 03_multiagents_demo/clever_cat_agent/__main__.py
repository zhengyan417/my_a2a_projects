...
def main(host:str="localhost", port:int=10000):
    ...
    encode_skill = AgentSkill(
        id='encode',
        name='解密工具',
        description='帮助将明文加密得到密文',
        tags=['decode', 'encode'],
        examples=['将"Beijing University of Posts and Telecommunications"这串字符串解密']
    )
    # 2. 定义AgentCard
    agent_card = AgentCard(
        name='Clever Cat Agent',
        description='帮助进行密文的解密或者明文的加密',
        ...
        skills=[decode_skill, encode_skill],
    )
